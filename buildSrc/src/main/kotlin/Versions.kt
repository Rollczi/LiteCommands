object Versions {

    // Core
    const val JETBRAINS_ANNOTATIONS = "26.0.2-1"

    // Tests
    const val JUNIT_JUPITER = "5.14.1"
    const val ASSERTJ = "3.27.6"
    const val MOCKITO = "5.20.0"
    const val AWAITILITY = "4.3.0"

    // Bukkit
    const val SPIGOT_API = "1.20.2-R0.1-SNAPSHOT"
    const val FOLIA_API = "1.21.4-R0.1-SNAPSHOT"
    const val PROTOCOL_LIB = "5.3.0"

    // BungeeCord
    const val BUNGEECORD_API = "1.21-R0.4"

    // JDA
    const val JDA = "6.1.1"

    // Minestom
    const val MINESTOM = "2025.10.05-1.21.8"

    // Sponge
    const val SPONGE_API = "16.0.0"

    // Velocity
    const val VELOCITY_API = "3.4.0-SNAPSHOT"

    // Fabric
    // check these on https://fabricmc.net/develop
    const val FABRIC_YARN_MAPPINGS = "1.21.11-pre2+build.3"
    val FABRIC_MINECRAFT = FABRIC_YARN_MAPPINGS.substringBefore('+')
    const val FABRIC_LOOM_PLUGIN = "1.13.6"
    const val FABRIC_LOADER = "0.18.1"
    const val FABRIC_COMMAND_API_V2 = "2.4.6+4ebb5c0879"
    const val FABRIC_COMMAND_API_V1 = "1.2.56+f71b366f73"
    const val FABRIC_LIFECYCLE_EVENTS_V1 = "2.6.15+4ebb5c0879"

    // TelegramBots
    const val TELEGRAM_BOTS = "9.2.0"

    // ChatGPT
    const val GSON = "2.13.2"
    const val CAFFEINE = "3.2.3"
    const val OKHTTP = "5.3.2"

    // Adventure
    const val ADVENTURE_MINIMESSAGES = "4.25.0"
    const val ADVENTURE_TEXT_SERIALIZER_LEGACY = "4.25.0"
    const val ADVENTURE_PLATFORM_API = "4.4.1"

    // Jakarta
    const val HIBERNATE_VALIDATOR = "9.0.1.Final"
    const val EXPRESSLY = "6.0.0"

    // LuckPerms
    const val LUCKPERMS_API = "5.5"

}
