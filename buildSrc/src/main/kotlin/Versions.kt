object Versions {

    // Core
    const val JETBRAINS_ANNOTATIONS = "26.0.2"

    // Tests
    const val JUNIT_JUPITER = "5.11.4"
    const val ASSERTJ = "3.26.3"
    const val MOCKITO = "5.18.0"
    const val AWAITILITY = "4.2.2"

    // Bukkit
    const val SPIGOT_API = "1.20.2-R0.1-SNAPSHOT"
    const val FOLIA_API = "1.21.4-R0.1-SNAPSHOT"
    const val SPIGOT = "1.8.8-R0.1-SNAPSHOT"
    const val PROTOCOL_LIB = "5.3.0"

    // BungeeCord
    const val BUNGEECORD_API = "1.20-R0.2"

    // JDA
    const val JDA = "5.5.1"

    // Minestom
    const val MINESTOM = "7320437640"

    // Sponge
    const val SPONGE_API = "13.0.0"

    // Velocity
    const val VELOCITY_API = "3.2.0-SNAPSHOT"

    // Fabric
    // check these on https://fabricmc.net/develop
    const val FABRIC_YARN_MAPPINGS = "1.21.6-pre4+build.1"
    val FABRIC_MINECRAFT = FABRIC_YARN_MAPPINGS.substringBefore('+')
    const val FABRIC_LOADER = "0.16.14"
    const val FABRIC_COMMAND_API_V2 = "2.2.53+946bf4c308"
    const val FABRIC_COMMAND_API_V1 = "1.2.56+f71b366f73"
    const val FABRIC_LIFECYCLE_EVENTS_V1 = "2.6.3+db4dfd8508"

    // TelegramBots
    const val TELEGRAM_BOTS = "8.2.0"

    // ChatGPT
    const val GSON = "2.11.0"
    const val CAFFEINE = "3.1.8"
    const val OKHTTP = "4.12.0"

    // Adventure
    const val ADVENTURE_MINIMESSAGES = "4.17.0"
    const val ADVENTURE_TEXT_SERIALIZER_LEGACY = "4.17.0"
    const val ADVENTURE_PLATFORM_API = "4.3.4"

    // Jakarta
    const val HIBERNATE_VALIDATOR = "9.0.1.Final"
    const val EXPRESSLY = "6.0.0"

    // LuckPerms
    const val LUCKPERMS_API = "5.4"

}